...
export class SolidSWR extends SWR {
  useSolid<D = any, E = Error>(
    ...
    const [state, setState] = createStore<SWRState<D, E>>({
...
export const createSWR = <D>(options?: Partial<SWROptions<D>>) => {
  ...
  return {
    useSWR: (key: SWRKey, options?: Partial<SWROptions<D>>) =>
      swr.useSolid(key, options),
    mutate: (key: SWRKey, value: D, options?: Partial<SWRMutateOptions<D>>) =>
      swr.mutate(key, value, options),
    revalidate: (key: SWRKey, options?: Partial<SWRRevalidateOptions<D>>) =>
      swr.revalidate(key, options),
    clear: (keys?: string | string[], options?: Partial<CacheClearOptions>) =>
      swr.clear(keys, options),
  };
};
